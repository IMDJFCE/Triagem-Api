...
@Component({
  selector: 'app-chips-behavorial-skill',
  templateUrl: './chips-behavorial-skill.component.html',
  styleUrls: ['./chips-behavorial-skill.component.scss'],
  standalone: true,
  imports: [MatFormFieldModule, MatChipsModule, MatIconModule,NgForOf],
})
export class ChipsBehavorialSkillComponent {
  addOnBlur = true;
  readonly separatorKeysCodes = [ENTER, COMMA] as const;
  skills: HabilidadeRequest[] = [{nome: 'Comunicação', tipo: HabilidadeTipo.COMPORTAMENTAL}, {nome: 'Inteligência Emocional', tipo: HabilidadeTipo.COMPORTAMENTAL}];
  ...
  add(event: MatChipInputEvent): void {
    ...
    if (value) {
      const newSkill: HabilidadeRequest = {
        nome: value,
        tipo: HabilidadeTipo.COMPORTAMENTAL
      };
